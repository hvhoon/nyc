...
@synthesize receivedHTTPHeaders;
...
{
    NSInteger responseCode = CFHTTPMessageGetResponseStatusCode(receivedHTTPHeaders);
    ...
    if(![self _checkHandshake:receivedHTTPHeaders]) {
...
{
    if (receivedHTTPHeaders == NULL) {
        receivedHTTPHeaders = CFHTTPMessageCreateEmpty(NULL, NO);
    }
    ...
    [self _readUntilHeaderCompleteWithCallback:^(SRWebSocket *self,  NSData *data) {
        ...
        if (CFHTTPMessageIsHeaderComplete(receivedHTTPHeaders)) {
            SRFastLog(@"Finished reading headers %@", CFBridgingRelease(CFHTTPMessageCopyAllHeaderFields(receivedHTTPHeaders)));
...
{
    SRFastLog(@"Connected");
    CFHTTPMessageRef request = CFHTTPMessageCreateRequest(NULL, CFSTR("GET"), ( CFURLRef)_url, kCFHTTPVersion1_1);
    
    // Set host first so it defaults
    CFHTTPMessageSetHeaderFieldValue(request, CFSTR("Host"), ( CFStringRef)(_url.port ? [NSString stringWithFormat:@"%@:%@", _url.host, _url.port] : _url.host));
    
    [_urlRequest.allHTTPHeaderFields enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
        CFHTTPMessageSetHeaderFieldValue(request, ( CFStringRef)key, ( CFStringRef)obj);
    }];
    ...
    CFHTTPMessageSetHeaderFieldValue(request, CFSTR("Sec-WebSocket-Key"), ( CFStringRef)_secKey);
    CFHTTPMessageSetHeaderFieldValue(request, CFSTR("Sec-WebSocket-Version"), ( CFStringRef)[NSString stringWithFormat:@"%d", _webSocketVersion]);
    CFHTTPMessageSetHeaderFieldValue(request, CFSTR("Origin"), ( CFStringRef)_url.absoluteString);
...
{    
    ...
    if (_secure) {
        ...
        [_outputStream setProperty:[NSDictionary dictionaryWithObject:[NSNumber numberWithBool:YES] 
                                                               forKey:( id)kCFStreamSSLAllowsAnyRoot]
